...
<div class="wrap">
	<h2>My First Wordpress Plugin Option</h2>
	...
	<form method="post" action="options.php">
		...
		<p>
			...
			<label class="description" for="mfwp_settings[enable]"><?php _e('Display the Foloow Me on Twitter link','mfwp_domain');?>
			</label>
		...
		<p>
			...
			<label class="description" for="mfwp_settings[twitter_url]">
				<?php _e('Enter your Twitter URL','mfwp_domain');?>
		...
		<?php $styles= array('blue','red');?>
			...
			<?php if($mfwp_options['theme'] == $style){
		...
		<p class="submit">
			<input type="submit" class="button-primary" value="<?php _e('Save Options','mfwp_domain');?>" name="">
		</p>
...
function mfwp_add_option_link(){
	add_options_page('My First Wordpress Plugin Option','My First Plugin','manage_options','mfwp_options','mfwp_options_page');
}

add_action('admin_menu','mfwp_add_option_link');
